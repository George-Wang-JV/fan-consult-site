...
- 管理员禁言 / 解除禁言 / 踢人
- 管理员置顶群消息
- 私聊与群聊支持照片、视频、语音和位置消息
- 长按消息可添加表情回应、撤回自己的消息或仅为自己删除
- SQLite 本地数据库
- Socket.IO 实时通信
...
聊天媒体文件保存在 `data/uploads/`，图片最大 12MB、视频最大 50MB、语音最大 15MB。请把整个 `data/` 目录纳入服务器备份；该目录不会提交到 GitHub。照片/视频上传、语音录制和位置共享都需要 HTTPS，首次使用语音或位置时浏览器会请求相应权限。
